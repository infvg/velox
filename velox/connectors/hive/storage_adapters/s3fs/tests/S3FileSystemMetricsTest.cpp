...
TEST_F(S3FileSystemMetricsTest, retryMetricTest) {
  ...
  VELOX_ASSERT_THROW(
      readFile->pread(0, kDataContent.length()),
      "Failed to get S3 object due to: 'Network connection'. Path:'s3://metrics/test.txt', SDK Error Type:99, HTTP Status Code:-1, S3 Service:'Unknown', Message:'curlCode: 7, Couldn't connect to server;")
  EXPECT_EQ(s3Reporter->counterMap[std::string{kMetricS3GetObjectRetries}], 0);
  EXPECT_EQ(s3Reporter->counterMap[std::string{kMetricS3ClientRetries}], 0);
}
} // namespace facebook::velox::filesystems::test
