...
// Helper function to handle Date type conversion (INTEGER kind)
VectorPtr handleDateTypeConversion(
    const TypePtr& type,
    const std::string& value,
    memory::MemoryPool* pool,
    bool isDaysSinceEpoch) {
  int32_t days = 0;
  // For Iceberg, the date partition values are already in daysSinceEpoch form.
  if (isDaysSinceEpoch) {
...
// Helper function to handle Decimal type conversion (HUGEINT/BIGINT kind)
VectorPtr handleDecimalTypeConversion(
    const TypePtr& type,
    const std::string& value,
    memory::MemoryPool* pool) {
  int32_t precision = 0;
...
VectorPtr newConstantFromStringImpl(
  ...
  if constexpr (kind == TypeKind::INTEGER) {
    if (type->isDate()) {
      return handleDateTypeConversion(type, value.value(), pool, isDaysSinceEpoch);
    }
  }
  ...
  if constexpr (kind == TypeKind::HUGEINT || kind == TypeKind::BIGINT) {
    if (type->isDecimal()) {
      return handleDecimalTypeConversion(type, value.value(), pool);
